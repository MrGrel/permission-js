...
export class PermissionBuilder<S extends string, A extends BaseActions<S>, C extends BaseConditions<S>> {
  ...
  subscribedCheck<Subjects extends S, Action extends A[Subjects]>({
    ...
    setDestroy,
    ...args
  }: SubscribedCheckPermissions<Subjects, Action, C> & Destroy): boolean {
    setDestroy(() => {
      console.log('destroy')
  ...
  removeSubscribe({ subject, action, signal }: UnsubscribePermissions<S, A[S], C>) {
    const removeTrigger = (subject: S, action: A[S][number]) => {
      this.subscribeManager.remove(subject, action, signal)
    }
